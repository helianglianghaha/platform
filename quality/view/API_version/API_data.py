...
def saveApiTestCase(request):
    ...
    modelData=int(request.POST.get("modelData"))
    versionData=int(request.POST.get("versionData"))
    apiRequest=request.POST.get('apiRequest')
    apiName=request.POST.get("apiName")
    testaddCookiesValue=request.POST.get('addCookiesValue')
        ...
        for test_list in  dataList:
            ...
            for caseData in caseList:
                ...
                if caseData['testcookiesValue']=='true':
                    ...
                    #查询登录接口
                    sql="select * from quality_testapi where testapiname='登录' and Modelversion_id_id="+str(Modelversion_id_id)
...
def apiRequest(request):
    ...
    if addCookiesValue:
        ...
        responseData=requestObject(url,header,data,method,cookies=cookie,apiExtractName=apiExtractName,apiExtractExpression=apiExtractExpression,passWordFree=passWordFree,addPassWordFree=addPassWordFree).requestApi()
        ...
        responseData=requestObject(url,header,data,method,cookies='',apiExtractName=apiExtractName,apiExtractExpression=apiExtractExpression,passWordFree=passWordFree,addPassWordFree=addPassWordFree).requestApi()
...
def selectVersionList(request):
    ...
    sql='select Modelversion_id as value,modeldata_name as label from quality_modelversion where modeldata_id_id='+str(modeldata_name)
    data=commonList().getModelData(sql)
    return JsonResponse(data,safe=False)
@loginRequired
@msgMessage
